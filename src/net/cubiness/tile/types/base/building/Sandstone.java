...
public class Sandstone extends TileData {
	...
	public Sandstone() {
		super(Material.SANDSTONE, TextureFile.SANDSTONE.getTexture());
		Random rand = new Random();
		for (int y = 0; y <= 4; y++) {
			ArrayList<Location> row = new ArrayList<>();
			row.add(new Location(0, (float) y * 0.25f));
			for (int x = 1; x <= 3; x++) {
				row.add(new Location((float) x * 0.25f + rand.nextFloat() / 5f - 0.1f, (float) y * 0.25f));
			}
		...
		for (int y = 0; y <= 4; y += 1) {
			...
			for (int x = 0; x <= 4; x += 1) {
				color_row.add(new Color(200 + rand.nextInt(10), 150 + rand.nextInt(10), 90 + rand.nextInt(10)));
			}
		...
		for (int y = 0; y < textureMap.size() - 1; y++) {
			for (int x = 0; x < textureMap.get(y).size() - 1; x++) {
				Render.renderQuad(
						...
						textureMap.get(y).get(x + 1).plus(l).plus(new Location(0, 0.25f)),
						textureMap.get(y).get(x).plus(l).plus(new Location(0, 0.25f)),
						colors.get(y).get(x),
						true);
			}
		}
